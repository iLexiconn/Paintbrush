...
public class PaintbrushAPI {
        ...
        if (!world.isRemote) {
            for (int ring = 0; ring < radius; ring++) {
                for (int i = 0; i < 360; ++i) {
                    ...
                    if (addPaint(world, facing, hitX, hitY, hitZ, pos.getX(), pos.getY(), pos.getZ(), pX, pY, color)) {
                        j++;
                    }
                }
            }
    ...
    public static int removePaint(World world, BlockPos pos, float hitX, float hitY, float hitZ, EnumFacing facing, int radius) {
        int j = 0;
        if (!world.isRemote) {
            for (int ring = 0; ring < radius; ring++) {
                for (int i = 0; i < 360; ++i) {
                    ...
                    int pX = (int) (-Math.sin(rad) * ring);
                    int pY = (int) (Math.cos(rad) * ring);
                    if (removePaint(world, facing, hitX, hitY, hitZ, pos.getX(), pos.getY(), pos.getZ(), pX, pY)) {
                        j++;
                    }
                }
            }
    ...
    private static boolean addPaint(World world, EnumFacing facing, float hitX, float hitY, float hitZ, int blockX, int blockY, int blockZ, int paintX, int paintY, EnumChatFormatting color) {
        ...
        PaintedBlockEntity paintedBlock = getPaintEntity(world, new BlockPos(blockX, blockY, blockZ), true);
    ...
    private static boolean removePaint(World world, EnumFacing facing, float hitX, float hitY, float hitZ, int blockX, int blockY, int blockZ, int paintX, int paintY) {
        ...
        PaintedBlockEntity paintedBlock = getPaintEntity(world, new BlockPos(blockX, blockY, blockZ), false);
    ...
    private static PaintedBlockEntity getPaintEntity(World world, BlockPos pos, boolean createNew) {
